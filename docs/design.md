...
| 連絡先と参加者 | **別の人として扱う**。`bookings.contact_name` / `email` / `phone` が連絡を取る相手、`bookings.name` と `booking_attendees` が参加者。子どもの体験なら前者が保護者、後者が子ども。**メールの宛名は連絡先** |
| 定員 | 開催回ごとに設定。**単位は人数**（3 名申込 = 3 席消費） |
| 対象年齢 | イベントごとに上限・下限を設定（**両方とも任意**、片方だけも可）。予約時に入力された**すべての年齢**を判定し、1 人でも範囲外なら受け付けない。座席判定より前に行うので**キャンセル待ちにもならない**。既に受け付けた予約は範囲を変えても取り消さない（`max_party_size` と同じ扱い） |
| 予約受付の開閉 | 全体を手動スイッチ＋開始/終了日時（JST）で開閉。**手動OFFが最優先**。停止は「新規の申込を受け付けない」だけで、**開催情報は表示し続け**、管理画面の繰り上げ・キャンセルも止めない。判定は `BookingWindow`、設定は `settings` テーブル（**行が無ければ受付中** ＝ マイグレーションを当てても挙動が変わらない） |
| 満席時 | キャンセル待ちとして登録。**管理者が手動で繰り上げ** |
| キャンセルで空いた席 | **キャンセル待ちのもの**。待ちが 1 件でもあれば、空席があっても新規予約はキャンセル待ちになる（後から来た人が追い越さない）→ [空席はキャンセル待ちのもの](#空席はキャンセル待ちのものbookingservicewouldwaitlist) |
| 管理画面 | フル（会社・イベント・開催回の CRUD、申込一覧、CSV 出力、パスワードログイン） |
| 本人操作 | トークン付き URL で予約内容の確認とキャンセル |
| メール | 送信処理は作る。開発中はファイル出力、設定 1 行で SMTP に切替 |
| メールの一斉送信 | 宛先は全申込者／体験プログラム／開催回の 3 段階。**キャンセル済みは常に除外**、キャンセル待ちはチェックボックス。**メールアドレスで重複除去**（1 人が何件予約していても 1 通）。会社担当者は**自社の申込者のみ**で、その絞り込みはフォームではなく**セッションの所属会社**から決める。積むのは `mail_queue`（`category = 'bulk'`）で、**予約完了直後のインライン送信は `transactional` だけを送る**ため、キャンペーンが予約した人を待たせない。「今すぐ送信」は 200 件ずつの分割送信で、毎回残り件数を返す |
| 本番環境 | Linux レンタルサーバー |
...
### 空席はキャンセル待ちのもの（`BookingService::wouldWaitlist`）

繰り上げが手動である以上、**キャンセルから管理者の操作までの間、「空席があり、かつ待っている人がいる」開催回が必ず存在します**。この窓の間に空席を通常の販売に戻すと、後から予約した人が列の全員を追い越して確定します。キャンセル待ちの意味が失われるので、次の規則を置いています。

> **キャンセル待ちが 1 件でも存在する開催回では、空席があっても新規予約はキャンセル待ちになる。**

判定は `BookingService::wouldWaitlist(残席, 申込人数, 待ち件数)` に集約し、予約トランザクションと画面表示の両方がこれを呼びます（別々に判定すると、確認画面が「確定」と約束した予約がトランザクションでキャンセル待ちになる）。待ち件数はセッション行のロック下で数えます。列に対する追加・削除も同じロックを取るため、この数は挿入までの間に変化しません。

**代償**：「1 席空いたが、待っているのは 3 名の組だけ」のように**列の誰も使えない端席は、繰り上げ操作まで空いたまま**になります。これは意図的な選択です。もう一方の案（誰も入れない席は新規に売る）は席を無駄にしませんが、「3 名待ちの前で 1 席が新規に売れ、後で 2 席空いても合計 3 席に届かない」という不利益が待っている人に残ります。**既に並んでいる人への保証を優先**し、端席はダッシュボードの「繰り上げ候補のある開催回」で見えるようにしてあります。

表示側もこの規則に合わせています。待ちがいる開催回では「残り N 名」を出さず（キャンセル待ちしかできない枠に残席を出すのは同じ誤報）、「キャンセル待ち受付中」とし、一覧カードの空き数も**待ちのいる開催回を 0 として集計**します。
...
- **`\R` や `.` を UTF-8 に対して使うときは `/u` を必ず付ける。** `/u` が無いと `\R` はバイト単位で動き、**0x85（NEL）を改行として扱います**。0x85 は UTF-8 の継続バイトとしてごく普通に現れ（`態` = E6 **85** 8B、`者` = E8 80 **85**、`入` = E5 **85** A5）、日本語コメントが文字の途中で切断されます。`bin/migrate.php` の行分割がこれを踏んでおり、`者` を含むコメントを書いた SQL ファイルで「コメント行の後半が SQL として実行される」という壊れ方をしました
- 既存のマイグレーション 001〜006 はたまたま該当文字を含まず、症状が出るまで気づけませんでした。**「今のファイルでは動いている」は根拠になりません**
