...
**移動時間バッファ（`travel_buffer` 設定）**: 重なりとは別に、前後の予約との間隔が `minutes`（既定 15、0 で無効）**以下**の場合は「移動時間を考慮すると、この予約は間に合いません」と扱います。`block = false`（既定）では確認画面に警告を表示し、確定ボタンに confirm() ポップアップを付けたうえで申込は可能。`block = true` では確認画面で送信を封じ、かつ `BookingService` がトランザクション内（申込者ロック下）で拒否します。繰り上げも block 時のみ同じ検査を行います。間隔 0 分（隣接枠）は重なり規則上は許可のままですが、このバッファ検査には掛かります。判定は境界含む（ちょうど 15 分も対象）。
