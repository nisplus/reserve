# 運用手順

データの初期化と一括投入。スキーマそのものは [database.md](database.md)、設計判断の背景は [design.md](design.md) にあります。

Windows では `php` を `php.cmd` に読み替えてください（`$env:PHP_BIN` の設定が必要な場合があります）。

---

## 1. データの初期化

**どこまで消すかで手順が違います。** 消しすぎると戻せないので、まず下の表で選んでください。

| やりたいこと | コマンド | 残るもの |
|---|---|---|
| テスト予約だけ消す（**本番公開前の定番**） | `php bin/reset_bookings.php --yes` | 会社・イベント・開催回・管理アカウント |
| サンプルデータで作り直す | `php bin/migrate.php --fresh` → `php bin/seed.php` | なし（管理アカウントも再作成） |
| 何も入っていない状態にする | `php bin/migrate.php --fresh` | 空のテーブルのみ |

### 予約だけ消す

```
php bin/reset_bookings.php --dry-run     # 件数を確認するだけ
php bin/reset_bookings.php --yes         # 実行
```

`--yes` を付けない限り何も変更しません。

消えるのは `bookings` / `booking_attendees` / `booking_events` / `applicants` / `mail_queue` で、**同時に `event_sessions` の `confirmed_seats` と `waitlist_counter` を 0 に戻します**。

> ここが手作業でやってはいけない理由です。`DELETE FROM bookings` だけを実行すると座席カウンタが残り、誰も予約していないのに「満席」になります（不変条件(1) 違反）。このスクリプトは同一トランザクションでカウンタも戻し、最後に整合性を検査して結果を表示します。

保持したいものがあれば:

```
php bin/reset_bookings.php --yes --keep-applicants   # 申込者マスタを残す
php bin/reset_bookings.php --yes --keep-mail         # 送信済みメール履歴を残す
```

`--keep-mail` を付けた場合、`mail_queue.booking_id` は NULL に落とします（消えた予約を指したままにしないため）。

### サンプルイベントとテスト予約を消して、本番データを入れる

`bin/seed.php` が入れたサンプル（14社・56イベント・422開催回）とテスト予約をまとめて消し、**スキーマと事務局アカウントは残す**手順です。この状態から [イベントの一括投入](#2-イベント情報の一括投入) に進めます。

```sql
-- 実行前に必ずバックアップを取ってください:
--   mysqldump -u root -p booking > backup_$(date +%Y%m%d).sql

SET FOREIGN_KEY_CHECKS = 0;

TRUNCATE TABLE booking_attendees;
TRUNCATE TABLE booking_events;
TRUNCATE TABLE mail_queue;
TRUNCATE TABLE bookings;
TRUNCATE TABLE applicants;
TRUNCATE TABLE event_sessions;
TRUNCATE TABLE events;

-- 会社が消えると所属先を失うため、会社担当者アカウントも一緒に削除します。
-- 事務局アカウント（role='superadmin'）は残ります。
DELETE FROM admin_users WHERE role = 'company';

TRUNCATE TABLE companies;

SET FOREIGN_KEY_CHECKS = 1;
```

ファイルに保存して流す場合:

```bash
mysql -u root -p booking < wipe.sql
```

#### なぜこの順番と `FOREIGN_KEY_CHECKS = 0` が要るのか

- **子テーブルが先**。`FOREIGN_KEY_CHECKS = 0` の間は `ON DELETE CASCADE` が**働かない**ので、`booking_attendees` や `booking_events` を明示的に消さないと親だけ消えて孤児が残ります
- **`DELETE` ではなく `TRUNCATE`**。AUTO_INCREMENT が 1 に戻るので、投入し直した本番データの ID が 1 から始まります（検証済み）
- **`admin_users` の扱いが要注意**。`admin_users.company_id` は `companies` を参照しており、そのままだと `Cannot delete or update a parent row`（errno 1451）で止まります。かといって `FOREIGN_KEY_CHECKS = 0` のまま放置すると、**存在しない会社を指したアカウントが残ります**（実測で 2 件発生）。上の SQL はそれを避けるため会社担当者を先に削除しています
- **最後に必ず `SET FOREIGN_KEY_CHECKS = 1`**。戻し忘れると、そのセッションの間だけ整合性チェックが効きません

#### 実行後の確認

```sql
SELECT 'companies' t, COUNT(*) n FROM companies
UNION ALL SELECT 'events',         COUNT(*) FROM events
UNION ALL SELECT 'event_sessions', COUNT(*) FROM event_sessions
UNION ALL SELECT 'bookings',       COUNT(*) FROM bookings
UNION ALL SELECT 'admin_users',    COUNT(*) FROM admin_users;

-- 宙に浮いた参照が無いこと（0 件が正常）
SELECT COUNT(*) AS dangling
FROM admin_users u LEFT JOIN companies c ON c.id = u.company_id
WHERE u.company_id IS NOT NULL AND c.id IS NULL;
```

`admin_users` が 0 件になってしまった場合は、ログインできなくなる前に作り直してください。

```bash
php bin/create_admin.php admin <パスワード>
```

#### PHP のツールでも同じことができます

SQL を触りたくない場合は、こちらでも同じ結果になります（`--fresh` は全テーブルを作り直すので**事務局アカウントも消えます**。`seed.php` が新しい管理者を作り、パスワードを一度だけ表示します）。

```bash
php bin/migrate.php --fresh
php bin/seed.php --fresh   # サンプルも入れ直す場合
```

サンプルを入れずに空から始めたいなら `bin/seed.php` を実行せず、`bin/create_admin.php` で管理者だけ作ってください。

### 全部作り直す

```
php bin/migrate.php --fresh    # 全テーブルを DROP して再作成
php bin/seed.php               # サンプルデータ（14社 × 4イベント × 計422開催回）
```

`bin/seed.php` は**管理者アカウントを作り、パスワードを一度だけ表示します。** 控えてください。控え損ねたら:

```
php bin/create_admin.php admin <新しいパスワード>
```

### 実施後の確認

```
php tests/test_invariants.php     # 5 本すべて 0 行なら健全
```

---

## 2. イベント情報の一括投入

`bin/import_events.php` が CSV から会社・イベント・開催回をまとめて作ります。開催回の書き方は 2 通りあり、1 つのイベントで混ぜられます。

| 書き方 | 使う列 | 意味 |
|---|---|---|
| **生成** | 開始日時＋所要分＋間隔分＋回数 | 等間隔に N 回（管理画面の一括生成と同じ） |
| **直接指定** | 開始日時＋**終了日時** | その 1 回だけを、書いたとおりに |

**同じ「会社名＋イベント名」の行は 1 つのイベントとしてまとめられます。** 時間割が不揃いでも、1 回 1 行で書けます。

**イベント共通の項目（説明・会場・外部URL・予約不要・上限人数・公開・エリア）は、そのイベントのどの行に書いても構いません。**（1 回だけ書いてください。他の行は空欄にします）。2 つの行が同じ項目を**違う値**で書いた場合は、どちらが正か決められないためエラーで中止します。

**行ごとに変えられるのは開催回の情報だけ**です（開始日時・終了日時・所要分・間隔分・回数・**定員**）。

### 手順

```
php bin/import_events.php --template > events.csv   # ひな形を出力
（Excel などで編集）
php bin/import_events.php events.csv --dry-run      # 検証だけ
php bin/import_events.php events.csv                # 投入
```

### 列

| 列 | 必須 | 内容 |
|---|---|---|
| 会社名 | ○ | 既存と**完全に同じ表記**なら再利用、違えば新規作成（下の「会社名の一致」を参照） |
| エリア | | `east` / `south` / `north` / `main` のみ。他の値はエラー。空欄可 |
| イベント名 | ○ | 同じ会社に同名があると中止 |
| 説明 | | 改行可（セルを `"` で囲む） |
| 会場 | | |
| 外部URL | | `http://` または `https://` のみ |
| 予約不要 | | `1` なら予約を受け付けない。**開催回は書いても書かなくても構いません**（書けば公開側に時間割として表示され、予約ボタンは出ません） |
| 上限人数 | | 1 予約あたりの上限。空欄なら 20 |
| 保護者を含める | | `1` なら参加人数に付き添いの保護者も含める（来場人数＝定員）。**空欄なら含めない**＝参加人数は体験する方だけを数え、予約画面に「付き添いの人数」欄が出て、付き添いは定員を消費しません |
| 公開 | | 空欄なら公開。`0` で非公開 |
| 開始日時 | ○※ | 開始。`2027-03-01 10:00` |
| 終了日時 | △ | **直接指定のときだけ。** `2027-03-01 10:45` または時刻だけの `10:45`（開始日時と同じ日として扱います） |
| 所要分 | △ | **生成のときだけ。** 1 回の長さ（5〜600） |
| 間隔分 | | 回と回の間の休憩。空欄なら 0 |
| 回数 | | 生成する回数（1〜20）。空欄なら 1 |
| 定員 | ○※ | 1〜999。行ごとに変えられます |

※ 予約を受け付けるイベントには**開催回が最低 1 つ必要**です。予約不要のイベントでは**任意**で、書かない場合は開始日時から下をすべて空欄にします。
△ **終了日時と、所要分・間隔分・回数は同時に指定できません。** どちらの書き方かが決まらないため、エラーで中止します。

「終了日時」と「保護者を含める」の列は**任意**です。列ごと無い CSV（以前の形式）もそのまま読めます。

### 参加人数の数え方（保護者を含める）

同じ「参加人数」欄が、イベントによって別のものを数えます。

| 設定 | 参加人数の意味 | 予約画面 | 定員 |
|---|---|---|---|
| 空欄（既定） | 体験する方だけ | 「付き添いの人数」欄が出る | 参加人数のみ消費。**来場人数は定員を超えうる** |
| `1` | 来場する全員 | 付き添い欄なし | 来場人数と同じ |

前者は「子どもが工作し、保護者は横で見ている」教室、後者は「親子で一緒に見学する」ツアーが典型です。

**空欄のイベントでは、会場の収容や配布物は定員ではなく「参加人数＋付き添い」で見積もってください。** その数は管理画面の予約一覧・開催回一覧（来場見込み）と CSV の「来場人数」列に出ています。

### 会社名の一致

会社は**名前の文字列がそのまま一致するか**で判定します。1 文字でも違えば別の会社が作られます。実測した結果は次のとおりです。

| CSV の書き方 | 結果 |
|---|---|
| `株式会社坂井製作所` | 既存に一致 |
| 末尾に空白（半角・全角とも） | 既存に一致（前後の空白は無視されます） |
| `株式会社 坂井製作所`（途中に空白） | **エラーで中止** |
| `株式会社　坂井製作所`（全角空白） | **エラーで中止** |
| `株式会社坂井製作`（1 文字欠落） | 新規作成 ＋ **警告** |
| `株式会社サカイ製作所`（カナ表記） | 新規作成（機械的には別会社としか判断できません） |
| `(株)坂井製作所`（略記） | 新規作成（同上） |

対策として次の 3 つが働きます。

1. **空白・全角半角の違いしかない名前はエラーで中止**します。登録済みの表記に直してください
2. 登録済みの会社と **2 文字以内の違い**なら警告を出します（別会社として登録される旨を明示）
3. **新しく作る会社は、件数ではなく名前を一覧で表示**します

```
新しく作る会社 2 社:
  2 行目  株式会社坂井製作
  3 行目  まったく新しい会社
  ※ 登録済みの会社に足すつもりの行がここにあれば、会社名の表記が違っています。

確認してください:
  2 行目: 新しく作る会社「株式会社坂井製作」は登録済みの「株式会社坂井製作所」と 1 文字違いです。別の会社として登録されます
```

カナ表記や略記までは機械的に見分けられません。**`--dry-run` でこの一覧を必ず目で確認してください。**

なお**エリアは 4 種類からの選択**なので、1 文字違いは新しい値にはならず、その場でエラーになります。

### 安全策

- **ファイル全体を検証してから書き込みます。** 40 行目に誤りがあれば 1 行も入りません
- 書き込みは**単一トランザクション**です。途中で失敗しても中途半端に残りません
- **DB に**既に同じ「会社名／イベント名」があると**エラーで中止**します。二重投入で重複が増えません（ファイル内で同名の行が並ぶのは、同じイベントの開催回としてまとめられます）
- 同じイベントに**同じ開始日時の回が 2 つある**とエラーで中止します（行をコピーして時刻を直し忘れたとき）。エラーは**重複を持ち込んだ行**を指します
- イベント共通の項目を 2 つの行が**違う値**で書くとエラーで中止します
- 同じ会社に**違うエリア**を書いた行があるとエラーで中止します
- 会社のエリアは、**未設定のときだけ** CSV の値で埋めます。設定済みの値は上書きしませんが、CSV と食い違う場合は**警告を出します**（従来は黙って無視していました）
- Excel の「CSV UTF-8」形式で出力した BOM 付きファイルもそのまま読めます

### 投入例

```csv
会社名,エリア,イベント名,説明,会場,外部URL,予約不要,上限人数,公開,開始日時,終了日時,所要分,間隔分,回数,定員
株式会社サンプル製作所,east,工場見学ツアー,ラインをご案内します。,本社工場 A棟,https://example.com/tour,,5,1,2027-03-01 10:00,,45,15,6,20
株式会社サンプル製作所,east,手づくり体験教室,キーホルダーを作ります。,研修棟 2F,,,4,1,2027-03-01 10:00,11:30,,,,12
株式会社サンプル製作所,,手づくり体験教室,,,,,,,2027-03-01 13:00,13:20,,,,6
株式会社サンプル製作所,,手づくり体験教室,,,,,,,2027-03-01 14:00,14:45,,,,20
株式会社サンプル製作所,east,常設展示,当日直接お越しください。,展示ホール,https://example.com/exhibit,1,,1,,,,,,
株式会社サンプル製作所,east,実演,1 日 2 回の実演です。,特設ステージ,,1,,1,2027-03-01 11:00,11:20,,,,30
株式会社サンプル製作所,,実演,,,,,,,2027-03-01 15:00,15:20,,,,30
```

1 行目（工場見学ツアー）は**生成**で、10:00 から 45 分の回を 15 分あけて 6 回（10:00 / 11:00 / … / 15:00）、定員 20 名。

2〜4 行目（手づくり体験教室）は**直接指定**で、1 つのイベントに 90 分・20 分・45 分の回が 3 つ。定員も回ごとに 12 / 6 / 20 名と別々です。説明・会場・上限人数は 1 行目にだけ書き、続きの行は空欄にしています（どの行に書いても同じです）。

5 行目（常設展示）は**予約不要・開催時間なし**。終日やっていて知らせる時刻がない場合です。

6〜7 行目（実演）は**予約不要だが開催時間あり**。公開側には時間割として 11:00〜11:20 と 15:00〜15:20 が並び、予約ボタンは出ません。

### 投入後の確認

`--dry-run` は開催回の**時刻をそのまま並べて**表示します。日付や長さの取り違えはここで気づけます。

```
検証 OK: イベント 4 件（7 行）/ 開催回 11 件

新しく作る会社 1 社:
  2 行目  株式会社サンプル製作所
  ※ 登録済みの会社に足すつもりの行がここにあれば、会社名の表記が違っています。

  株式会社サンプル製作所  工場見学ツアー      6 回  03-01 10:00〜10:45  03-01 11:00〜11:45  …  ほか 2 件
  株式会社サンプル製作所  手づくり体験教室    3 回  03-01 10:00〜11:30  03-01 13:00〜13:20  03-01 14:00〜14:45
  株式会社サンプル製作所  常設展示            [予約不要] 開催時間なし
  株式会社サンプル製作所  実演                [予約不要] 2 回  03-01 11:00〜11:20  03-01 15:00〜15:20
```

---

## 2-2. 「保護者も参加人数に含める」を後から有効にする

チェックを入れ忘れたまま予約を受け付けてしまった場合の、既存予約の付け替えです。
`db/merge_guardians_into_party.sql` に手順と SQL が入っています。

```
mysqldump -u root -p booking > backup.sql          # 先にバックアップ
# ファイル冒頭の @event_id に対象イベントの ID を入れてから
mysql -u root -p booking < db/merge_guardians_into_party.sql
```

やっていること（1 トランザクション）:

1. `party_size += guardian_count` / `guardian_count = 0`（キャンセル済みも含む）
2. `event_sessions.confirmed_seats` を `SUM(party_size)` から**引き直す**（加算ではないので、何度実行しても同じ値になります）
3. `events.party_includes_guardians = 1`

**実行前に事前確認をすべて 0 行にしてください。** 特に次の 2 つは実際に起きます。

- **定員オーバー** — 変換前の定員は「体験する人の数」でした。保護者を足すと超える回があります。先に定員を上げるか、その回は変換しません
- **20 名の上限** — `chk_bookings_party` に弾かれて `UPDATE` が失敗します（トランザクションなので中途半端には残りませんが、`mysql --force` を付けると途中までコミットされてしまうので**付けないでください**）

**変換後に残る差**: 付き添いの方は氏名も年齢も聞いていないため、`booking_attendees` に行がありません。予約一覧は「人数 3・氏名 2 名分」のように表示されます（壊れはしません）。埋めるなら当日確認になります。

**定員の意味が変わります**: 変換後の定員は「来場する人の数」です。受け入れ人数を維持したいなら定員も上げてください。

---

## 2-3. 受付後に同行者を増やす／減らす

`db/add_attendee.sql` に手順と SQL が入っています。管理画面に予約の編集機能は無いため、現状はこれが唯一の方法です。

**`booking_attendees` に行を足すだけでは足りません。** このテーブルは「誰が来るか」の記録で、席を取っているのは `bookings.party_size` です。行だけ足すと名前はあるのに席が無い人ができます。

**しかもこれは不変条件を破りません。** `tests/test_invariants.php` は全項目 OK を返し、残席はその人の分を数えないまま売られ続け、当日その回が 1 人ぶん溢れます。管理画面には「人数 2／氏名 3 名」と食い違って表示されます（実測）。

席まで動かすには 3 つを 1 トランザクションで更新します。

1. `booking_attendees` に 1 行（`attendee_no` は既存の最大 + 1。`uq_attendees_slot` があるので重複させられません）
2. `bookings.party_size + 1`
3. `event_sessions.confirmed_seats + 1` — **`confirmed` の予約のときだけ**。キャンセル待ちはまだ席を取っていないので、ここを動かすと不変条件(1) が壊れます

加えて `booking_events` に記録を残します（状態は変わらないので `from` と `to` を同じにし、`note` に「人数 2 → 3」）。

### 実行前に確認すること

- **席があるか** — 満席なら増やせません
- **その回にキャンセル待ちがいないか** — 空席はお待ちの方のものです（[空席はキャンセル待ちのもの](design.md#空席はキャンセル待ちのものbookingservicewouldwaitlist)）。ここで既存予約に足すと、新規予約には回さないと決めた席をあとから申し出た同行者に回すことになり、**待っている方を追い越します**。足すかどうかは人の判断で
- **1 予約あたりの上限人数**と 20 名の上限
- キャンセル済みの予約には足さない

### 別のケース

- **増えるのが「体験しない付き添い」で、そのイベントが `party_includes_guardians = 0`** のとき: 席を取らないので `guardian_count + 1` だけ。`attendees` も `party_size` も触りません
- **減らすとき**: 逆向きに。`attendee_no` は詰めなくて構いません（欠番のままで正常）。`confirmed_seats` を戻し忘れると、その席は誰も取れないまま埋まり続けます

予約者には「N 名で受け付けました」とメールが届いています。人数変更は別途ご連絡ください（この SQL はメールを送りません）。
---

## 3. スキーマ資料の更新

マイグレーションを追加したら、テーブル定義書を再生成してください。

```
php bin/schema_doc.php           # docs/database.md を書き直す
php bin/schema_doc.php --check   # 最新でなければ終了コード 1
```

列の説明文は `bin/schema_doc.php` 内の `$columnNotes` に書きます（生成のたびに引き継がれます）。
