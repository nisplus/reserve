...
- **予約不要イベント** — 予約ボタンを出さず、外部サイトへのリンク（別タブ）を掲載できる。開催回は任意で、登録すれば公開側に**時間割として表示**されます（予約ボタンは出ません）
- **連絡先（ご氏名・メールアドレス・電話番号）と参加者（お名前・年齢）を別々に収集**。お子様の体験など、連絡を取る相手が参加者でない場合に対応します。確認メールの宛名は連絡先の方です
- 参加人数での予約。**1予約あたりの上限人数はイベントごとに設定**でき、2名以上なら全員のお名前と年齢を入力していただきます
- **イベントごとの対象年齢**（上限・下限とも任意、片方だけも可）。予約時に入力された年齢が範囲外なら「対象年齢の範囲外です」と表示して受け付けません。キャンセル待ちの登録も同じ扱いです。判定は**入力されたすべての年齢**が対象で、付き添いの保護者は参加人数に含める設定のときだけ対象になります（含めない設定では年齢を伺わないため）
- **参加人数の数え方をイベントごとに切替**。「付き添いの保護者も含めて数える」イベントと、「体験する方だけを数え、付き添いは別に登録する」イベントの両方に対応。後者の付き添いは**定員を消費せず**、来場人数（参加人数＋付き添い）を管理画面・CSV に表示します
- **開催企業へのメッセージ**（任意・1000文字以内）を予約時に添えられます。管理画面・CSV・確認メールに反映されます
- イベントに**外部リンクURL**を設定すると、予約画面とイベント詳細に「開催企業の紹介ページで見る」として表示されます（予約不要のイベントに限りません。未設定なら非表示）
...
- **予約受付の全体停止**（管理画面「受付設定」）。手動スイッチと受付開始・終了日時（**JST**）を併用でき、手動OFFが最優先。停止中も**体験内容・開催時間・会場は表示されたまま**で、予約ボタンだけが消えます。締切をまたいで送信された予約もトランザクション内で拒否します。管理画面の繰り上げ・キャンセルは停止中も操作できます
- **メールの一斉送信**（管理画面「一斉送信」）。天災などによる中止・延期のお知らせ用です。宛先は**全申込者／体験プログラム指定／開催回指定**から選び、`キャンセル待ちの方も含める` を切替。**キャンセル済みの方には送りません**。同じメールアドレスは何件予約していても 1 通だけです。体験内容と開催日時は本文の先頭に自動で付きます。**会社担当者は自社の申込者にだけ**送れます（全申込者は事務局のみ）。自分宛のテスト送信 → 件数確認 → 確認ポップアップ → キュー投入、という順で、キューは「今すぐ送信」で 200 件ずつ送れます（残り件数が毎回表示されます）
- 開催回ごとの定員管理（単位は人数）と、満席時のキャンセル待ち
- **キャンセルで空いた席はキャンセル待ちの方のもの**。キャンセル待ちが1件でもある開催回では、空席があっても新規予約はキャンセル待ちとして受け付けます（後から予約した人が待っている人を追い越しません）。繰り上げは自動では行わず、管理画面から操作します
- トークン付き URL による本人での予約確認・キャンセル
- 管理画面（会社／イベント／開催回の CRUD、申込一覧、繰り上げ、CSV 出力）。**予約一覧は「予約日時順」と「会社・体験プログラム・開催日時順」を切り替え**でき（後者は同じ開催回の中で確定→キャンセル待ち→キャンセル済みの順）、選んだ並び順はページ送りと CSV 出力にも引き継がれます
- **管理画面にも公開側と同じ空き状況**を表示。開催回一覧は回ごと（残り N 名／満席／キャンセル待ち）、体験プログラム一覧は全開催回を集計したもの（空き N 名分／全回満席／キャンセル待ち／受付前）。定員と確定席だけでは分からない「空席はあるがキャンセル待ちが押さえている」状態が事務局からも見えます
...
php.cmd tests/test_area_filter.php     # エリア絞り込み（共有URL・未設定の扱い）
php.cmd tests/test_age_limit.php       # 対象年齢（境界値・片方だけ・キャンセル待ち・付き添いの扱い）
php.cmd tests/test_booking_window.php  # 予約受付の開閉（既定は受付中・境界・停止中も繰り上げ可）
php.cmd tests/test_bulk_mail.php       # メール一斉送信（重複除去・キャンセル済み除外・他社に届かないこと）
php.cmd tests/test_booking_sort.php    # 予約一覧の並び順（5つのキーの優先順位・ページ送りの安定性）
php.cmd tests/test_event_availability.php  # 空き状況バッジの集計（4状態・公開側と管理側の一致）
php.cmd tests/test_guardians.php       # 付き添い人数（座席を消費しないこと・来場人数）
php.cmd tests/test_contact.php         # 連絡先氏名と参加者氏名の分離（メールの宛名）
php.cmd tests/test_queue_priority.php  # 空席はキャンセル待ちのもの（追い越し防止）
...
「事務局には所属会社を持たせない／会社担当者には必ず持たせる」という対応関係は `AdminUserRepository` が全書き込み経路（管理画面・`bin/create_admin.php`）で強制します。当初は CHECK 制約でしたが、MariaDB 11.8 が受け付けないため（errno 1901）アプリ側に移しました。`tests/test_authz.php` が検証しています。
